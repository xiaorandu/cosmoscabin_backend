...
import com.project.onlinebooking.exception.ReservationCollisionException;
import com.project.onlinebooking.exception.ReservationNotFoundException;
...
public class ReservationService {
    ...
    @Transactional(isolation = Isolation.SERIALIZABLE) // make sure reservation belongs to this user, then can delete
    public void delete(Long reservationId, String username) {
        ...
        for (LocalDate date = reservation.getCheckinDate(); date.isBefore(reservation.getCheckoutDate()); date = date.plusDays(1)) {
            stayReservationDateRepository.deleteById(new StayReservedDateKey(reservation.getStay().getId(), date)); // if object, need to new an object
        }
        reservationRepository.deleteById(reservationId); //primary key
    }
}
